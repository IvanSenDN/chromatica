...
async fn main() -> Result<()> {
    ...
    let html = to_file_url("onbeforeunload.html");
    page.navigate(&html, None, None).await?;

    let element = page
        .wait_for_selector("a[href='https://www.google.com']", None)
        .await?;
    let (_, dialog) = tokio::join!(
        element.click(),
        page.wait_for_js_dialog(|dialog| dialog.is_prompt() != true, None,)
    );
    dialog.unwrap().accept(None).await?;
    println!("Onbeforeunload accepted");
